...
int BinarySearchTree<ItemType>::getHeightHelper(BinaryNode<ItemType>* subTreePtr) const
{
	...
	// add your code here
	int L = getHeightHelper(subTreePtr->getLeftChildPtr());
	int R = getHeightHelper(subTreePtr->getRightChildPtr());
	return 1 + max(L, R);
}
...
int BinarySearchTree<ItemType>::getHeightDifference()
{
	...
	// add your code here
	return getHeightHelper(rootPtr->getLeftChildPtr()) - getHeightHelper(rootPtr->getRightChildPtr());
}
...
int BinarySearchTree<ItemType>::nodeDegree_I(BinaryNode<ItemType>* subtreePtr, const ItemType target)
{
	...
	while (!q.empty()) {
		...
		if (n->getLeftChildPtr() != nullptr) {
			q.push(n->getLeftChildPtr());
		}
		if (n->getRightChildPtr() != nullptr) {
			q.push(n->getRightChildPtr());
		}
	}
...
int BinarySearchTree<ItemType>::nodeDegree_R(BinaryNode<ItemType>* subtreePtr, const ItemType target)
{
	...
	if (subtreePtr != nullptr)
	{
		...
		int L = nodeDegree_R(subtreePtr->getLeftChildPtr(), target);
		int R = nodeDegree_R(subtreePtr->getRightChildPtr(), target);
		if (L != -1) {
			return L;
		}
		if (R != -1) {
...
int BinarySearchTree<ItemType>::singleChildCount_R(BinaryNode<ItemType>* subtreePtr)
{
	...
	if (subtreePtr != nullptr)
	{
			...
			cnt += singleChildCount_R(subtreePtr->getLeftChildPtr());
			cnt += singleChildCount_R(subtreePtr->getRightChildPtr());
			return cnt;
	}
