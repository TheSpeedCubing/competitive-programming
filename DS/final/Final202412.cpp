...
void pqArrayInsert(BinaryNode<string>* pqArray, int& size, const string word)
{
	...
	pqArray[size++] = *(new BinaryNode<string>(word));
	return;
}
...
int pqArrayMaxWordCountNo(BinaryNode<string>* pqArray, int& size, int index)
{
	...
	int cnt = 0;
	int maxCnt = pqArrayMaxWordCount(pqArray, size, 0);

	for (int i = 0; i < size; i++) {
		if (pqArray[i].getCount() == maxCnt) {
			cnt++;
		}
	}
	return cnt;
...
BinaryNode<string> pqArrayMaxWord(BinaryNode<string>* pqArray, int& size)
{
	...
	for (int i = 0; i < size; i++) {
		if (pqArray[i].getCount() > entry.getCount()) {
			entry = pqArray[i];
		} else if (pqArray[i].getCount() == entry.getCount() && pqArray[i].getItem() > entry.getItem()) {
			entry = pqArray[i];
...
int RemoveMaxDegreeEdges(int** matrixGraph, int numVertices)
{
	...
	vector<int> degArr(numVertices, 0);
	int maxDeg = 0;
	for (int i = 0; i < numVertices; i++) {
		int deg = 0;
		for (int j = 0; j < numVertices; j++) {
	...
	for (int i = 0; i < numVertices; i++) {
		if (degArr[i] == maxDeg) {
			for (int j = 0; j < numVertices; j++) {
				if (matrixGraph[i][j] != 0 && matrixGraph[i][j] != INT_MAX) {
					matrixGraph[i][j] = INT_MAX;
					matrixGraph[j][i] = INT_MAX;
					removed++;
				}
			}
		}
...
void doQ3()
{
	...
	while (1) {
		...
		if (sInput == "A" || sInput == "a") {
			...
			if (degree < 0)
				cout << endl << aString << " does not exist. (Iteration)";
			else if ( degree == 0)
				cout << endl << aString << " is a leaf node. (Iteration)";
			else
				cout << endl << aString << " is not a leaf node. (Iteration)";

			degree = myBST.nodeDegree("r", aString);

			if (degree < 0)
				cout << endl << aString << " does not exist. (Recursion)";
			else if (degree == 0)
				cout << endl << aString << " is a leaf node. (Recursion)" << endl;
			else
				cout << endl << aString << " is not a leaf node. (Recursion)" << endl;
		}
		else if (sInput == "S" || sInput == "s") {
			cout << endl << "The iterative signle child node count of the BST is " << myBST.singleChildCount("i");
			cout << endl << "The recursive signle child node count of the BST is " << myBST.singleChildCount("r") << endl;
		}
