...
int countR(string s, char c)
{
	...
	return (s[0] == c) + countR(s.substr(1), c);
}
...
char findR(string s) {
	...
	return findR(s[1] < s[0] ? s.substr(1) : s[0] + s.substr(2));
}
...
string removeR(string s) {
	...
	if (s[0] == s[1]) {
		return removeR(s.substr(1));
	} else return s[0] + removeR(s.substr(1));
}
